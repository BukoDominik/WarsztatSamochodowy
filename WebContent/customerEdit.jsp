...
<%@ include file="templates/header.jsp" %>
	...
	<%	
			List<Customer> customersField = (List<Customer>) request.getAttribute("customers");
 			String idString = request.getAttribute("idOfEdit").toString();
 			int id = Integer.parseInt(idString);
 			System.out.println(id);
			for (Customer customer : customersField) {
    ...
    <label>ID <input class="form-control" disabled placeholder=<%=customer.getId()%> /></label><br />
	</div>
	<div class="form-group">
	<label>Imie  <input class="form-control" placeholder=<%=customer.getFirstName()  %> name='firstName'/></label><br/>
	</div>
	<div class="form-group">
	<label>Nazwisko<input class="form-control" placeholder=<%=customer.getSecondName()  %> name='secondName'/></label><br/> 
	</div>
	<div class="form-group">
	<label>Data urodzenia(MM/dd/yyyy) <input class="form-control" name='birthday' type='date'/></label><br/>
	</div>
	<input type='submit' class="btn btn-primary" name="act" value='update'/> 
	</form>
	</div>
			<%
			 } }
		...
		<div class="card well col-sm-3" style="width: 18rem;">
			<div class="card-body">
				...
				<input type="submit" value="delete" name="act" id="box_button" class="btn btn-primary col-md-6">
				<input type="submit" value="edit" name="act" id="box_button" class="btn btn-primary col-md-6">
				</form>
			</div>
		</div>
		<%
